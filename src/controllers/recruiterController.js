...
const createJob = async (req, res) => {
    const { title, description, companyId, requirement, benefit, province_city, detailed_location, working_time, due_to, salary, experience, level, quantity, working_type, sex, professionId } = req.body;
    // Kiem tra du lieu dau vao:
    if (!title || !description || !requirement || !working_time || !salary || !level || !quantity || !working_type || !companyId || !province_city || !professionId) {
        return res.status(400).json({ message: "Please enter all field!" });
    }
    try {
        const newJob = new Job({
            title, description, companyId, requirement, benefit, province_city, detailed_location, working_time, due_to, salary, experience, level, quantity, working_type, sex, professionId
        });
...
const createCompany = async (req, res) => {
    const { name, logo, website, number_of_employee, introduction, location } = req.body;
    //kiem tra du lieu dau vao:
    if (!name || !logo) {
        return res.status(400).json({ message: "Please enter all field!" });
    }
    try {
        ...
        const newCompany = new Company({
            name, logo, website, number_of_employee, introduction, location
        });
        ...
        //ok:
        return res.status(201).json({ message: 'Saved successfully', savedCompany });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Saved failed!' });
    }
...
const jobList = async (req, res) => {
    try {
        ...
        console.error(error);
        return res.status(500).json({ message: 'Cannot get job list from database!' });
    }
}
...
const filterJobByExperience = async (req, res) => {
    ...
    if (!selectedExperience) {
        return res.status(400).json({ message: 'Experience parameter is required' });
    }
    try {
        const jobs = await Job.aggregate([
            {
...
const filterJobByProfession = async (req, res) => {
    ...
    if (!selectedProfession) {
        return res.status(400).json({ message: 'Profession parameter is required' });
    }
